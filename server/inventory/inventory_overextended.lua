...
if Config.Inventory and Config.Inventory == 2 then
    ...
    RegisterNetEvent('rcore_gangs:server:open_ox_inventory', function(playerId)
        local source = source
        ...
        if #(pedCoords - targetPedCoords) > 5.0 then
            return
        end

        exports.ox_inventory:forceOpenInventory(source, 'player', playerId)
    end)
end