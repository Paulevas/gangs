...
RegisterKeyMapping(Config.Commands.GANGMENU or 'gangmenu', 'Gang Menu', 'keyboard', Config.KeyBinds.gangMenu or 'f10')
        ...
        if zone and zone.gangPresence then
            for index, gang in ipairs(zone.gangPresence) do
                ...
                if Gang and gang.name == Gang.name then
                    local doesPermExist = Config.RanksAccess['rivalry.give_up_territory'] ~= nil
                    if doesPermExist then
                        if (Gang.access and Gang.access['rivalry.give_up_territory']) or Gang.leader then
                    ...
                    elseif Menu.Button('GIVE_UP_TERRITORY', Locale.MENU_GIVE_UP_TERRITORY, "", true) then
                        Menu.CloseMenu()
                        TriggerServerEvent("rcore_gangs:server:giveTerritoryUp")
                    end
                end
